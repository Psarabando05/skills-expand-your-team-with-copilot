...
def get_activities(
    ...
    - difficulty: Filter activities by difficulty level ('Beginner', 'Intermediate', 'Advanced').
                  Activities with no difficulty set are included when filtering, as they are open to all levels.
    """
    ...
    if difficulty:
        # Include activities that match the difficulty OR have no difficulty set (open to all levels)
        difficulty_condition = {"$or": [
            {"difficulty": difficulty},
            {"difficulty": {"$exists": False}}
        ]}
        # Combine with existing query using $and to avoid overwriting other filters
        if query:
